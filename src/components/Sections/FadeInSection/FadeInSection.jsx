...
const FadeInSection = ({ title, imageSrc, imageAlt, text = [], linkText, linkHref }) => {
  const [ref, inView] = useInView({
    ...
    rootMargin: "-100px 0px", // Start the animation slightly before the element is in view
  });

  return (
    <div ref={ref} className={`transition-all duration-1500 transform ${inView ? "scale-100 opacity-100" : "opacity-0 scale-50"}`} style={{ transitionDuration: "1750ms" }}>
      <h2 className="text-3xl mb-12 text-center font-bold">{title}</h2>
      <div className="md:flex md:space-x-6 p-5">
        <div className="md:w-1/2 mb-5">
          <img src={imageSrc} alt={imageAlt} className="rounded-3xl" />
        </div>
        <div className="md:w-1/2 p-1 md:p-5 flex flex-col text-lg">
          {text &&
