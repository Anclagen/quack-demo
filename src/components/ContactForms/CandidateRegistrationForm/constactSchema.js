...
export const allAddressSchema = Yup.object().shape({
  ...
  "previous-address-1": Yup.string().when("yearsAtAddress", {
    is: "yes",
    then: () => Yup.string().required("Previous Address Line 1 is required."),
  }),
...
export const availabilitySchema = Yup.object().shape({
  ...
  "available-from": Yup.date().required("Available from date is required").min(new Date(), "Start date cannot be in the past"),
  "emergency-name": Yup.string().required("Emergency contact name is required").max(100, "Emergency contact name cannot exceed 100 characters"),
  "emergency-relationship": Yup.string().required("Emergency contact relationship is required").max(100, "Emergency contact relationship cannot exceed 100 characters"),
...
export const bankDetailsSchema = Yup.object().shape({
  ...
  "account-number": Yup.string().required("Account number is required").max(8, "Account number cannot exceed 8 characters"),
  "sort-code": Yup.string().required("Sort code is required").max(6, "Sort code cannot exceed 6 characters"),
  "bank-branch": Yup.string().required("Bank branch is required").max(100, "Bank branch cannot exceed 100 characters"),
});
...
export const referenceDetailsSchema = Yup.object().shape({
  ...
  "ref-employed-from": Yup.date().required("Employed from date is required").max(new Date(), "Employed from date cannot be in the future"),
  "ref-employed-to": Yup.date().required("Employed to date is required").max(new Date(), "Employed to date cannot be in the future"),
  "ref-reason-leaving": Yup.string().required("Reason for leaving is required").max(100, "Reason for leaving cannot exceed 100 characters"),
  "ref-name": Yup.string().required("Name is required").max(100, "Name cannot exceed 100 characters"),
