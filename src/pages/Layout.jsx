        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <ul className="hidden sm:flex space-x-5 pr-2">
            {[
              ...
              <li key={route.path}>
                <Link to={route.path} className={`${location.pathname === route.path ? "underline" : ""} hover:underline hover:text-white text-white text-lg`} onClick={handleMenuClose}>
                  {route.label}
                </Link>
              </li>
            ))}
        ...
        <div ref={menuRef} className={`fixed top-0 right-0 h-full bg-black bg-opacity-90 w-64 transform transition-transform ${isMenuOpen ? "translate-x-0" : "translate-x-full"} sm:hidden`}>
          ...
          <ul className="flex flex-col space-y-4 p-4">
            {[
              ...
              <li key={route.path}>
                <Link to={route.path} className={`${location.pathname === route.path ? "underline" : ""} hover:underline hover:text-white text-white`} onClick={handleMenuClose}>
                  {route.label}
                </Link>
              </li>
            ))}
